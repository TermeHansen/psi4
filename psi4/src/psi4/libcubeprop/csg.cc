...
//    auto local_block = std::make_shared<BlockOPoints>(offset, xp, yp, zp, wp, extents_);
	return std::make_shared<BlockOPoints>(0, offset, xp, yp, zp, wp, extents_);
}
...
void CubicScalarGrid::add_density(double* v, std::shared_ptr<Matrix> D, const int d) {
...
void CubicScalarGrid::compute_density(std::shared_ptr<Matrix> D, const std::string& name, const int& cube_divider, const std::string& type) {

    double* v = new double[npoints_];
    memset(v, '\0', npoints_ * sizeof(double));
    add_density(v, D, cube_divider);
    ...
    double total_elecs = C_DASUM(npoints_, v,1)*D_[0]*D_[1]*D_[2];
    std::cout << "\nNumber of electrons: " << total_elecs << "\n";
