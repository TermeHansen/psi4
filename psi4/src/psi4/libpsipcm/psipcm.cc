...
PCM::PCM(const std::string &pcmsolver_parsed_fname, int print_level, std::shared_ptr<BasisSet> basisset)
    ...
    auto tess_charges_n = std::make_shared<Vector>(tesspi_); // HFlabs
    pcmsolver_get_surface_function(context_.get(), ntess_, tess_charges_n->pointer(0), ASC_n_label.c_str()); // HFlabs
...
double PCM::compute_E_total(const SharedVector &MEP_e) const {
    ...
    auto ASC = std::make_shared<Vector>(tesspi_); // HFlabs
    pcmsolver_get_surface_function(context_.get(), ntess_, ASC->pointer(0), ASC_label.c_str()); // HFlabs
    pcmsolver_save_surface_functions(context_.get()); // HFlabs
...
double PCM::compute_E_separate(const SharedVector &MEP_e) const {
    ...
    auto ASC_n = std::make_shared<Vector>(tesspi_); // HFlabs
    pcmsolver_get_surface_function(context_.get(), ntess_, ASC_n->pointer(0), ASC_n_label.c_str()); // HFlabs
    pcmsolver_save_surface_functions(context_.get()); // HFlabs
    ...
    auto ASC_e = std::make_shared<Vector>(tesspi_); // HFlabs
    pcmsolver_get_surface_function(context_.get(), ntess_, ASC_e->pointer(0), ASC_e_label.c_str()); // HFlabs
    pcmsolver_save_surface_functions(context_.get()); // HFlabs
